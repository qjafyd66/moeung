...
export default function AdminPage() {
  const [menu, setMenu] = useState<"events" | "crawl">("events");
  ...
  return (
    <>
      ...
      <div className="flex gap-2 mb-6">
        {[
          { id: "events" as const, label: "이벤트 관리", count: events.length },
          { id: "crawl" as const, label: "크롤링 대기", count: crawlQueue.length },
        ].map((item) => (
          <button
            key={item.id}
          ...
          >
            ...
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
              menu === item.id ? "bg-white/30 text-white" : "bg-gray-100 text-gray-500"
            }`}>
              {item.count}
            </span>
          </button>
      ...
      {/* 이벤트 관리 */}
      {menu === "events" && <div className="flex flex-col md:flex-row gap-6">
          {/* ── 왼쪽: 이벤트 등록/수정 폼 ── */}
