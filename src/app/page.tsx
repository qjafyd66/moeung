...
export default function Home() {
  ...
  const [legalModal, setLegalModal] = useState<"terms" | "privacy" | "location" | "marketing" | "notice" | "support" | null>(null);
      ...
      <footer className="border-t border-gray-100 mt-8 py-8 px-4">
        <div className="max-w-3xl mx-auto flex flex-col gap-4">
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            ...
            <button onClick={() => setLegalModal("notice")} className="text-xs text-text-muted hover:text-primary-400 transition-colors">공지사항</button>
            <button onClick={() => setLegalModal("support")} className="text-xs text-text-muted hover:text-primary-400 transition-colors">고객센터</button>
          </div>
